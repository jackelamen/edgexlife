    ...
    <div className="streak-chart-wrap">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ overflow: 'visible' }}>
        ...
        {yTicks.map((v) => {
          ...
          return (
            <g key={v}>
              <text x={padL - 4} y={y + 3} textAnchor="end" fontSize="8" fontWeight="700" fill="var(--text-3)">{v}</text>
              <line x1={padL} y1={y} x2={startX + totalW} y2={y} stroke="var(--border)" strokeWidth="1" />
            </g>
          )
        })}
        {scores.map((s, i) => {
          ...
          if (s === null) {
            return (
              <g key={i}>
                <rect x={x} y={H - padB - 6} width={barW} height={6} rx={3} fill="var(--white-soft)" />
                <text x={x + barW / 2} y={H - 1} textAnchor="middle" fontSize="9" fontWeight="700" fill="var(--text-3)">{i + 1}</text>
              </g>
            )
          }
          const barH = Math.max(6, Math.round((s / 100) * (H - padB - 6)))
          const y = H - padB - barH
          const color = barStatus(s).color
          return (
            <g key={i}>
              ...
              <text x={x + barW / 2} y={H - 1} textAnchor="middle" fontSize="9"
                fontWeight={isCur ? '800' : '700'} fill={isCur ? color : 'var(--text-3)'}>{i + 1}</text>
            </g>
          )
        })}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8, flexWrap: 'wrap' }}>
        {[[STATUS.good.color, '≥85% Excellent'], [STATUS.short.color, '65–84% Solid'], [STATUS.risk.color, '<65% Push']].map(([c, l]) => (
